...
def non_max_suppression_fast(boxes, overlap_thresh=0.9, max_boxes=300):
    ...
    # test if x1 values are smaller than x2 and y1 smaller than y2
    np.testing.assert_array_less(x1, x2)
    np.testing.assert_array_less(y1, y2)

    # if boxes are signed integer then convert them to float
    # this is important as we are going to do some divisions
