...
import random
import copy
from models.faster_rcnn import data_augment
import threading
import itertools
...
def calc_rpn(cfg, img_data, width, height, resized_width, resized_height, img_length_calc_function):
	...
	downscale = float(cfg['rpn']['stride'])
			...
			for ix in range(output_width):
				...
				for jy in range(output_height):
					...
					for bbox_num in range(num_bboxes):
						...
						if curr_iou > best_iou_for_bbox[bbox_num] or curr_iou > cfg['rpn']['max_overlap']:
						...
						if img_data['bboxes'][bbox_num]['class'] != 'Background':
							...
							if curr_iou > cfg['rpn']['max_overlap']:
								bbox_type = 'pos'
							...
							if cfg['rpn']['min_overlap'] < curr_iou < cfg['rpn']['max_overlap']:
...
def get_anchor_gt(all_img_data, class_count, cfg, img_length_calc_function, backend, mode='train'):
	...
	while True:
		...
		for img_data in all_img_data:
			try:
				...
				(resized_width, resized_height) = get_new_img_size(width, height, cfg['img']['min_side'])
				...
				x_img[:, :, 0] -= cfg['img']['channel_mean'][0]
				x_img[:, :, 1] -= cfg['img']['channel_mean'][1]
				x_img[:, :, 2] -= cfg['img']['channel_mean'][2]
				x_img /= cfg['img']['scaling_factor']

				x_img = np.transpose(x_img, (2, 0, 1))
				x_img = np.expand_dims(x_img, axis=0)

				y_rpn_regr[:, y_rpn_regr.shape[1]//2:, :, :] *= cfg['std']['scaling']
